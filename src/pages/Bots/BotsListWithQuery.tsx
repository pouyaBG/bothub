...
import { useBots, useDeleteBot, useToggleBotStatus } from "../../hooks/useBots";
...
const BotsListWithQuery: React.FC = () => {
  ...
  const handleToggleStatus = async (id: number, currentStatus: string) => {
    ...
    try {
      await toggleBotStatus.mutateAsync({ id, status: newStatus });
    } catch (error) {
      console.error('خطا در تغییر وضعیت بات:', error);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-8">
        ...
        <p className="text-center text-gray-600 mt-4">در حال بارگذاری بات‌ها...</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm p-8">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">خطا در بارگذاری داده‌ها</h3>
          <p className="text-gray-600 mb-4">امکان دریافت اطلاعات بات‌ها وجود ندارد</p>
        ...
        {filteredBots.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              {searchTerm ? 'جستجوی خود را تغییر دهید' : 'اولین بات خود را اضافه کنید'}
            </p>
            {!searchTerm && (
              <Button
                variant="primary"
